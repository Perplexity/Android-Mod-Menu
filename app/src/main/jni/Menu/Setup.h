...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle){
    //Set sub title
    setText(env, title, OBFUSCATE("<b>Modded by ThePerplexedOne</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<p style=\"font-size:30\">"
                                     "<p style=\"color:green;\">0.0.0</p></p>"));

    //Dialog Example
    //setDialog(ctx,env,OBFUSCATE("Title"),OBFUSCATE("Message Example"));

    //Toast Example
    Toast(env,ctx,OBFUSCATE("Modded by ThePerplexedOne @ platinmods.com"),ToastLength::LENGTH_LONG);
